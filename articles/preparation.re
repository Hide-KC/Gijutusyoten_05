...
KotlinはJavaのいいとこどりみたいな言語ですが、その特徴を見ていきます。
...
従来通りの「var s: T」は、Kotlinではすべてnullを入れられない非許容型として扱われます。
...
許容型で宣言しても通常通りメソッドを呼ぶ方法が２種類あります。
...
T? -> Tにスマートキャスト（@<hd>{smart_cast}）されるため、Tのメソッドを呼ぶことができます。
...
JvmOverloadsアノテーションを付与して実装すると微妙に動作がおかしくなる件もあります。
